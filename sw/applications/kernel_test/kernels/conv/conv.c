...
cgra_input[3][0] = &buffer[0][0];
    ...
    for(int16_t i = 0; i < 3; i++) {
        cgra_input[0][i] =   &(input[0][0][i][0]);
        cgra_input[1][i] =   &(input[0][0][i][1]);
        cgra_input[2][i] =   &(input[0][0][i][2]);

}

    for(int16_t i = 3; i < 6; i++){
        cgra_input[0][i] = filter[0][0][i-3][0];
        cgra_input[1][i] = filter[0][0][i-3][1];
        cgra_input[2][i] = filter[0][0][i-3][2];
    }
...
uint32_t check(void)
{
    ...
    for(int i = 0; i < row_output; i++){
        for(int j = 0; j < col_output; j++){
            if(outputs[0][0][i][j] != buffer[i][j]){
                errors++;
                printf("Error at %d %d\n", i, j);
                printf("Expected %d, got %d\n", outputs[0][0][i][j], buffer[i][j]);
            }
        }
    }
    if(errors == 0){
